...
export default function Projects() {
  ...
  const projects: Project[] = [
    {
      title: 'LLMFuzz: Distributed LLM-Guided Fuzzing Agent',
      description: 'Built a coverage-guided fuzzing system that uses an LLM-powered ReAct loop with adaptive strategy selection to automatically generate high-quality test inputs and discover crashes in Python programs. Outperformed random fuzzing by 7.6 percentage points (98.3% vs 90.7% branch coverage) using 53% fewer inputs, finding 4 crashes a random baseline missed.\n\nThe system scales across stateless workers via a Redis Streams consumer group architecture, enabling zero-config horizontal scaling. A coordinator monitors per-target coverage in real time and auto-reassigns low-coverage targets with alternative fuzzing strategies.\n\nCore architecture:\n\n- Coverage-guided ReAct loop using Claude SDK for intelligent input generation and strategy adaptation\n- Redis Streams-based distributed task queue with consumer groups for parallel fuzzing across workers\n- Adaptive strategy selection: the coordinator detects coverage plateaus and switches strategies (e.g., boundary, mutation, type-aware) automatically\n- Real-time branch coverage tracking via coverage.py with per-target metrics\n- Stateless worker design allowing horizontal scaling without configuration changes\n- Crash deduplication and reproduction with detailed stack traces\n\nKey results:\n\n- 98.3% branch coverage vs 90.7% for random fuzzing baseline\n- 53% fewer inputs needed to achieve higher coverage\n- 4 unique crashes discovered that the random baseline missed\n- Linear scaling with additional workers on multi-target campaigns',
      images: ['/project-images/LLMFuzz_1.png'],
      technologies: ['Python', 'Claude SDK', 'Redis Streams', 'coverage.py', 'Distributed Systems', 'ReAct Loop', 'Fuzzing'],
      github: 'https://github.com/YohanV1/LLMFuzz',
      demo: '#',
      featured: true,
